...
func (sess *wsSession) claim(which feed, subID string) uint64 {
	gen, _ := sess.tryClaim(which, subID, mk1)
	return gen
}

func TestTheSubscriptionLimitCountsBothFeedsAndAllowsReplacement(t *testing.T) {
	sess := &wsSession{
		tables: map[string]*entry{},
		logs:   map[string]*entry{},
	}
	...
	for index := range maxSubscriptions {
		...
		if _, claimed := sess.tryClaim(which, fmt.Sprintf("s%d", index), mk1); !claimed {
			t.Fatalf("subscription %d was refused before the limit", index)
		}
	}
	if _, claimed := sess.tryClaim(tables, "extra", mk1); claimed {
		t.Fatal("a subscription beyond the connection limit was accepted")
	}
	if _, claimed := sess.tryClaim(tables, "s0", mk2); !claimed {
		t.Fatal("replacing an existing subscription consumed another slot")
	}

	sess.drop(streams, "s1")
	if _, claimed := sess.tryClaim(tables, "after-drop", mk1); !claimed {
		t.Fatal("unsubscribing did not release a slot")
	}
}
